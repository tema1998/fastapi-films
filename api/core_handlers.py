...
@core_router.get('/genres')
async def get_genres(db: AsyncSession = Depends(connect_db)):
    genres = await _get_genres(db)
    if len(genres) == 0:
        raise HTTPException(status_code=404, detail=f"There is no genres yet.")
    return genres